    ...
    async fillTodosList(todos) {
        for (const todo of todos) {
            ...
            if(todoItem)
            {
                ...
                checkboxDiv.classList.add("h5");
                checkboxDiv.classList.add("text-primary");
                ...
                button_cha.classList.add('btn-outline-info');
                button_cha.classList.add('button_mar');
                ...
                if(task_status === 0)
                {
                    ...
                    chk.checked = true;
                    todoItem.classList.add("strike_out");
                    headerBlock.classList.add("strike_out");
                ...
                chk.addEventListener('change', async () => {
                    ...
                    if(chk.checked)
                    {
                        ...
                        todoItem.classList.remove("strike_out");
                        headerBlock.classList.remove("strike_out");
                        entry = {
                            task_id: todo.id,
                            action: "uncompleted"
                        };
                ...
                button_rem.classList.add('button_mar');
        ...
        try{
            ...
            var newModal = `
                <div class="modal fade" id="edit-todo-window${id}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                   <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <button type="button" class="btn btn-outline-primary" id='save-todo-edit${id}'>Сохранить изменения</button>
                        </div>
                    </div>
                    </div>
                </div>
                ...
                $('#save-todo-edit'+id).on('click', async () => {
                    if ($('#edit-name'+id).val() == '' || $('#edit-description'+id).val() == '') {alert("Заполните все поля."); return;}
                ...
                if (new_name==data['name'] && new_description==data['description']) {$('#edit-todo-window'+id).modal('hide'); return;}
                    ...
                    const entry = {
                     ...
                     $('[data-target="#demo'+id+'"]')[0].innerText=new_name;
                     location.reload();
                }});
        }
        catch (error) {
